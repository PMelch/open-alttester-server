#!/usr/bin/env node
/**
 * Cross-runtime CLI entry for open-uitester-server.
 *
 ...
 * directly:  bun ./bin/open-uitester-server.ts
 */
...
const tsEntry = join(__dirname, "open-uitester-server.ts");
...
child.on("error", (err) => {
  console.error(`open-uitester-server: failed to start — ${err.message}`);
  process.exit(1);
});
