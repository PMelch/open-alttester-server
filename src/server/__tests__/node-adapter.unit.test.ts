...
import { createUiTesterServer } from "../server";
...
// ---------------------------------------------------------------------------
// AC#4: createUiTesterServer adapter injection
// ---------------------------------------------------------------------------

describe("createUiTesterServer — adapter injection (AC#4)", () => {
  it("accepts a NodeServerAdapter via opts.adapter and returns a working server", async () => {
    ...
    const srv = await createUiTesterServer({ port: 0, adapter: nodeAdapter });
