...
export class NodeServerAdapter implements ServerAdapter {
  ...
  private async handleRequest(
    ...
    const req = await toRequest(nodeReq, this.port);
    try {
  ...
  private handleUpgrade(
    ...
    const request = toRequestWithoutBody(req, this.port);
...
async function toRequest(req: IncomingMessage, port: number): Promise<Request> {
  ...
  return new Request(requestUrl(req, port), init);
}

function toRequestWithoutBody(req: IncomingMessage, port: number): Request {
  return new Request(requestUrl(req, port), requestInit(req));
}

function requestUrl(req: IncomingMessage, port: number): string {
  return `http://127.0.0.1:${port}${req.url ?? "/"}`;
}

function requestInit(req: IncomingMessage): RequestInit {
  ...
  return { method: req.method ?? "GET", headers };
}
